...
@post_app.route("/like", methods=["POST"])
@login_required
async def like() -> Tuple["Response", int]:
    error: bool = False
    ...
    if request.method == "POST":
        ...
        if not error:
            ...
            # insert like on like table
            like_record = {
                "uid": str(uuid.uuid4()),
                "parent_post_id": parent_post_id,
                "user_id": session.get("user_id"),
            }
            like_query = like_table.insert().values(**like_record)
            await conn.execute(query=like_query)
